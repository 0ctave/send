...
SendStream.prototype.isMalicious = function(){
  return !this._root.length && ~this.path.indexOf('..');
};
...
SendStream.prototype.pipe = function(res){
  ...
  // checks next root dir
  var next = function() {
    var root = rootDirs[i],
        isLast = i >= rootDirs.length - 1,
        curPath = path

    // join / normalize from optional root dir
    if (root) curPath = normalize(join(root, curPath));

    // ".." is malicious without "root"
    if (self.isMalicious()) return self.error(403);

    // malicious path
    if (root && 0 != curPath.indexOf(root)) return self.error(403);

    // hidden file support
    if (!self._hidden && self.hasLeadingDot()) return self.error(404);

    // index file support
    if (self._index && self.hasTrailingSlash()) curPath += self._index;

    i++

    debug('stat "%s"', curPath);
    fs.stat(curPath, function(err, stat){
      if (err) return isLast ? self.onStatError(err) : next();
      if (stat.isDirectory()) return self.redirect(self.path);
      self.send(curPath, stat);
    });
  }
